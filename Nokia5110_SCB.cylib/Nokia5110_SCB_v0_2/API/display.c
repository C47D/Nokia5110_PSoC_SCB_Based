...
#endif //DRAW_OVER_BACKGROUND
{
    ...
    if (0 == `$SPI_NAME`_initVar) {
        `$SPI_NAME`_Start(); // Enable SPI
    }
    
    `$SPI_NAME`_SpiSetActiveSlaveSelect(`$SPI_NAME`_SPI_SLAVE_SELECT0);
    // `$INSTANCE_NAME` Extended Commands.
    `$INSTANCE_NAME`_SendCommand(0x21);
    // Set `$INSTANCE_NAME` Vop (Contrast).
    `$INSTANCE_NAME`_SendCommand(0xC0);
    // Set Temp coefficent.
    `$INSTANCE_NAME`_SendCommand(0x06);
    // `$INSTANCE_NAME` bias mode 1:48.
    `$INSTANCE_NAME`_SendCommand(0x13);
    // `$INSTANCE_NAME` Standard Commands, Horizontal addressing mode.
    `$INSTANCE_NAME`_SendCommand(0x20);
    // `$INSTANCE_NAME` in normal mode.
    `$INSTANCE_NAME`_SendCommand(0x0C);
    ...
    if(background == NULL){
        ...
        for(uint16_t n = 0; n < 504; n++){
            background = 0x00;
            `$INSTANCE_NAME`_SendData(0x00);
        } //84 pixel x 6 lines = 504 
    }else{
        Background = background;
        `$INSTANCE_NAME`_SendDataLen(Background, 504);
        Background_cursor = 0;
    }
#else //DRAW_OVER_BACKGROUND
...
void `$INSTANCE_NAME`_Chr(char ch, DRAW_TYPE_t dtype, FONT_TYPE_t ft)
{
    ...
    case Small_Font:
        if(dtype == DRAW_OR){
            for(uint16_t i = 0; i < 3; i++, Background_cursor++) {
    ...
    default:// Default Middle font
        if(dtype == DRAW_OR){  
            for(uint16_t i = 0; i < 5; i++, Background_cursor++){
...
* Return value : None.
***************************************************************************/
void `$INSTANCE_NAME`_Image(uint8_t x, uint8_t y, uint8_t xsize,
    uint8_t ysize, const char* dataPtr)
{
    ...
    uint8_t yposition, DRAW_TYPE_t dtype)
{
    ...
    for(uint8_t i = 0; i < 6 ; i++, Background_cursor++){
        switch(yposition){
    ...
    for(uint8_t j = yend - 1; j > ystart; j--){
        if(yposition > 8){
            yposition -=8;
        }else{
            yposition=0;
...
void `$INSTANCE_NAME`_HBargraph(uint8_t y, uint8_t xstart, uint8_t xend, uint8_t xposition, DRAW_TYPE_t dtype){
    ...
    for(uint8_t i = 1; i < xposition; i++, Background_cursor++)
        if(dtype == DRAW_OR){
            `$INSTANCE_NAME`_SendData(0x7E | Background[Background_cursor]);
        }else if(dtype == DRAW_XOR){
            `$INSTANCE_NAME`_SendData(0x7E ^ Background[Background_cursor]);
    ...
    for(uint8_t i = xposition + 1; i < xend - xstart; i++, Background_cursor++)
        if(dtype == DRAW_OR){
...
void `$INSTANCE_NAME`_HBargraph(uint8_t y, uint8_t xstart, uint8_t xend, uint8_t xposition){
    ...
    for(uint8_t i = 1; i <= xposition; i++){
        `$INSTANCE_NAME`_SendData(0x7E);
    }
    for(uint8_t i = xposition+1; i < xend - xstart; i++){
...
void `$INSTANCE_NAME`_SendData(uint8_t data)
{
    ...
    `$SPI_NAME`_SpiUartWriteTxData(data);

    while(`$SPI_NAME`_SpiIsBusBusy()) {
    }
}

void `$INSTANCE_NAME`_SendDataLen(uint8_t *data, size_t data_len)
{
    `$RST_NAME`_Write(1);
    `$DC_NAME`_Write(1);    
    
    for (size_t idx = 0; idx < data_len; idx++) {
        `$SPI_NAME`_SpiUartWriteTxData(data[idx]);
        while(`$SPI_NAME`_SpiIsBusBusy()) {
        }
    }
}
...
void `$INSTANCE_NAME`_SendCommand(uint8_t command)
{
    ...
    `$SPI_NAME`_SpiUartWriteTxData(command);
    while(`$SPI_NAME`_SpiIsBusBusy()) {
    }
}
